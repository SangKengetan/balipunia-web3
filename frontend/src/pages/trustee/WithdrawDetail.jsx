...
function renderAmountSnapshot(snapshot) {
  ...
  const lockedRate = data.locked_rate || null;
      ...
      {lockedRate && (lockedRate.usdt_idr > 0 || lockedRate.usdc_idr > 0) && (
        <div className="p-3 bg-indigo-50 rounded-lg border border-indigo-100">
          <p className="text-xs font-bold text-indigo-700 mb-1.5 flex items-center gap-1">
            ...
            Kurs Terkunci (Saat Pengajuan)
          </p>
          <div className="flex gap-4 text-xs text-indigo-600">
            <span>1 USDT = Rp {Number(lockedRate.usdt_idr).toLocaleString("id-ID")}</span>
            <span>1 USDC = Rp {Number(lockedRate.usdc_idr).toLocaleString("id-ID")}</span>
          </div>
          {lockedRate.locked_at && (
            <p className="text-[10px] text-indigo-400 mt-1">
              Dikunci: {new Date(lockedRate.locked_at).toLocaleString("id-ID")} • Sumber: CoinGecko
            </p>
          )}
        </div>
      )}
